...
void OrbMatcher::SetDepthImages(const cv::Mat &first, const cv::Mat &second) {
  first_depth_img_ = first.clone();
  second_depth_img_ = second.clone();
  return;
}
...
std::vector<cv::DMatch> OrbMatcher::Run() {
  ...
  cv::FAST(first_gray_img_, keypoints, 40);
  cout << "keypoints: " << keypoints.size() << endl;

  vector<cv::Point2f> image_points(0);

  for (auto i = 0; i < keypoints.size(); ++i) {
    float z = GetPointDepth(first_depth_img_, keypoints[i].pt);
    if (z <= 0) {
  ...
  ComputeAngle(first_gray_img_, keypoints);

  // compute ORB descriptors
  vector<DescType> descriptors;
  ComputeORBDesc(first_gray_img_, keypoints, descriptors);
  ...
  // compute angle for each keypoint
  ComputeAngle(second_gray_img_, keypoints2);

  // compute ORB descriptors
  vector<DescType> descriptors2;
  ComputeORBDesc(second_gray_img_, keypoints2, descriptors2);
...
// Return depth.
float OrbMatcher::GetPointDepth(const cv::Mat &depth_image,
...
void OrbMatcher::BfMatch(const vector<DescType> &desc1,
                         const vector<DescType> &desc2,
                         vector<cv::DMatch> &matches) {
  int d_max = 50;
...
// compute the angle
void OrbMatcher::ComputeAngle(const cv::Mat &image,
                              vector<cv::KeyPoint> &keypoints) {
...
void OrbMatcher::ComputeORBDesc(const cv::Mat &image,
                                vector<cv::KeyPoint> &keypoints,
